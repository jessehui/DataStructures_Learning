...
### 最小生成树
和图类似, 但是每两个节点之间的值不再是1, 而是有各自的长度. 求最小生成树, 要求遍历所有点所用的总长度最小, 即所有点联通(从任意一点, 可以去其他的任意一点)所用的总长度最小.
实现的话要用到一个`并查集`的数据结构(Disjoint set), 是一种树形的数据结构. 这个数据结构应该提供的方法: 查找(find, 返回根节点), 合并(union). 
